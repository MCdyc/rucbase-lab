...
DiskManager::DiskManager()
{
    ...
    std::fill(fd2pageno_, fd2pageno_ + MAX_FD, 0); // 初始化fd2pageno_数组
}
...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes)
{
    ...
    // if (fcntl(fd, F_GETFD) < 0) // 检查文件是否打开
    // {
    //     throw FileNotOpenError(fd);
    // }
    if (lseek(fd, page_no * PAGE_SIZE, SEEK_SET) < 0) // 定位到指定页面
    {
        throw UnixError();
    }
...
void DiskManager::deallocate_page(__attribute__((unused)) page_id_t page_id)
{
    // 目前不支持释放页面
    // 未来可以考虑使用回收站的方式来实现
    // throw InternalError("DiskManager::deallocate_page Error: deallocation not supported");
}
...
void DiskManager::close_file(int fd)
{
    ...
    if (fd < 0 || fd >= MAX_FD || !fd2path_.count(fd)) // 此处已检查是否为end
    ...
    path2fd_.erase(iter->second); // 从path2fd_中删除该文件路径
    fd2path_.erase(iter);         // 从fd2path_中删除该文件句柄
    fd2pageno_[fd] = 0;           // 重置该文件的已分配页面个数
}
